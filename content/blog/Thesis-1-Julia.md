...
_Pluto_ поддерживает также письмо юникодом - например, если хочется написать греческую букву альфа, нужно написать `\alp` и нажать *TAB*, после чего выпадет список с подсказками, выбор варианта из которого свернет предыдущий escape-code до нужного символа.

# Установка

Для работы достаточно скачать компилятор языка [здесь](https://julialang.org/install/), в интерфейс которого можно попасть командой julia в терминале. 

Для установки пакетов нужно так или иначе вызвать местный пакетный менеджер (например, нажать в интерфейсе компилятора `]`) и написать `add ИМЯ`.  В принципе, из пакетов можно поставить только `Pluto`, а всё остальное ставить уже на доски.
