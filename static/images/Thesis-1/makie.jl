...
function build_surface3_makie(;
	...
	gruv_cmap = cgrad(
    	gruv_colors,            # Массив цветов
    	#[0.0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0], # Позиции
		[0.0,1.1],
    	categorical = false,    # Непрерывный градиент
    	rev = false             # Порядок цветов
	)
    param_labels = Dict(
        :Ta => L"T_a",
        :Tπ => L"T_\pi",
	...
	with_theme(theme_latexfonts()) do
		...
		surf = surface!(ax, X, Y, Z, rasterize = 10)
		Colorbar(
			fig[1, 2],
			surf,
